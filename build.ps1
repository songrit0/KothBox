# Builds KothBox.dll without the .NET SDK, using the Roslyn C# compiler that
...
# (i.e. no standalone .NET SDK installed). Output: bin\KothBox.dll
#
...
$refs = @(
    ...
    "$proj\lib\0Harmony.dll", "$proj\lib\MySql.Data.dll",
    "$fx\System.dll", "$fx\System.Core.dll", "$fx\System.Xml.dll", "$fx\System.Data.dll"
)

New-Item -ItemType Directory -Force "$proj\bin" | Out-Null
$srcs = Get-ChildItem $proj -Filter *.cs | Select-Object -ExpandProperty FullName  # Exclude subdirs
$cscArgs = @("/nologo", "/target:library", "/langversion:latest", "/optimize+",
    "/out:$proj\bin\KothBox.dll") + ($refs | ForEach-Object { "/reference:$_" }) + $srcs

& $csc $cscArgs
if ($LASTEXITCODE -eq 0) { Write-Host "OK -> $proj\bin\KothBox.dll" -ForegroundColor Green }
